...
if command -v walrus &> /dev/null; then
    if walrus store /tmp/test_walrus.txt --epochs 1 > /tmp/walrus_cli.log 2>&1; then
        ...
        echo "   ❌ Walrus CLI FAILED"
        echo "   Check: cat /tmp/walrus_cli.log"
        FAILED=$((FAILED + 1))
    fi
else
    echo "   ❌ Walrus CLI NOT INSTALLED"
    FAILED=$((FAILED + 1))
fi
echo ""
...
if command -v sui &> /dev/null; then
    ...
    if [ -n "$BALANCE" ] && [ "$BALANCE" -gt 0 ]; then
        ...
        echo "   ✅ Wallet OK (~$BALANCE_SUI SUI)"
    else
        echo "   ⚠️  Low/No balance - get testnet SUI from faucet"
        FAILED=$((FAILED + 1))
    fi
else
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "🔧 Check these issues before demo:"
