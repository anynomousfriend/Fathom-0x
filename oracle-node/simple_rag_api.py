...
def query_gemini(context, question):
    ...
    try:
        ...
        url = f"{base_url}/gemini-pro:generateContent?key={GEMINI_API_KEY}"

        prompt_text = (
            "Based on the following document context, answer the "
...
def query_document():
    ...
    try:
        ...
        if not encrypted_data:
            error_msg = 'Failed to download document from Walrus'
        ...
        relevant_chunks = simple_semantic_search(
        ...
        if not answer:
            error_msg = (
                'No LLM configured. Please set GEMINI_API_KEY or '
                'OPENAI_API_KEY'
            )
            return jsonify({'error': error_msg}), 500
...
def query_mock():
    ...
    base = "This is a mock RAG response. Based on the document analysis, "

    if 'conclusion' in question.lower() or 'main' in question.lower():
        answer = (
            base + "the main conclusion is that the proposed approach "
            "shows significant improvements over baseline methods."
        )
    elif 'method' in question.lower():
        answer = (
            base + "the methodology involves a transformer-based "
            "architecture with attention mechanisms."
        )
    elif 'result' in question.lower():
        answer = (
            base + "the key results show a 15% improvement on "
            "benchmark datasets."
        )
    else:
        answer = (
            base + "the document discusses various aspects of the "
            "research topic with detailed analysis and findings."
        )
