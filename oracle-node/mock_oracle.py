...
This is a simplified mock oracle that simulates the TEE oracle
functionality for demonstration purposes. It doesn't require the full
pysui SDK or TEE hardware.
...
MOCK_DOCUMENTS = {
    ...
    "research_paper": {
        ...
        Abstract:
        This paper presents a novel approach to privacy-preserving
        machine learning using Trusted Execution Environments (TEEs)
        and blockchain technology.

        Key Findings:
        1. TEE-based inference maintains 99.8% accuracy vs unprotected
        2. Blockchain integration ensures audit trails and provenance
        3. Cryptographic attestation provides verifiable privacy
...
MOCK_RESPONSES = {
    "financial": {
        "summary": (
            "This financial report shows strong Q4 2024 performance with "
            "$10.5M revenue (25% YoY growth) and $2.1M net profit (30% "
            "growth). Key highlights include enterprise segment expansion, "
            "new product launches, and 95% customer retention."
        ),
        "key_points": (
            "The key points are: 1) Revenue of $10.5M with 25% "
            "year-over-year growth, 2) Net profit of $2.1M with 30% growth, "
            "3) Successful expansion into 3 new markets, 4) High customer "
            "retention at 95%, and 5) Positive outlook with 20% projected "
            "growth for Q1 2025."
        ),
        "conclusion": (
            "The main conclusion is that the company demonstrated strong "
            "financial performance in Q4 2024 with significant revenue and "
            "profit growth, successful market expansion, and a positive "
            "outlook for continued growth."
        ),
        "metrics": (
            "Key metrics from this report: Revenue: $10.5M (+25% YoY), "
            "Net Profit: $2.1M (+30% YoY), Operating Expenses: $8.4M, "
            "Customer Retention: 95%, Projected Q1 2025 Growth: 20%."
        ),
    },
    "medical": {
        "summary": (
            "This is a patient medical record for a 45-year-old individual "
            "with hypertension and type 2 diabetes. Currently on Metformin "
            "and Lisinopril with stable condition and good control "
            "(HbA1c 6.8%)."
        ),
        "conditions": (
            "The patient has two main chronic conditions: Hypertension "
            "(diagnosed in 2020) and Type 2 Diabetes (diagnosed in 2018). "
            "There are no known drug allergies."
        ),
        "medications": (
            "Current medications include Metformin 500mg taken twice daily "
            "for diabetes management, and Lisinopril 10mg taken once daily "
            "for blood pressure control."
        ),
        "status": (
            "The patient's current health status is stable. Recent blood "
            "work shows HbA1c at 6.8%, which indicates good diabetes "
            "control. Routine checkup on December 1st, 2024 confirmed "
            "stable condition."
        ),
    },
    "research": {
        "summary": (
            ...
            "privacy-preserving machine learning using TEEs and blockchain. "
            "The study demonstrates that TEE-based inference maintains 99.8% "
            "accuracy while providing cryptographic privacy guarantees."
        ),
        "findings": (
            "The key findings are: 1) TEE-based inference maintains 99.8% "
            "accuracy compared to unprotected models, 2) Blockchain "
            "integration ensures complete audit trails and data provenance, "
            "and 3) Cryptographic attestation provides verifiable privacy "
            "guarantees."
        ),
        "methodology": (
            "The research methodology involved deploying ML models in Intel "
            "SGX enclaves, using homomorphic encryption for data "
            "preprocessing, and validating the approach on HIPAA-compliant "
            "healthcare datasets."
        ),
        "conclusion": (
            "The main conclusion is that TEE-blockchain integration "
            "successfully enables secure, privacy-preserving AI for "
            "sensitive data while maintaining both model performance and "
            "full auditability."
        ),
    }
}
...
class MockOracle:
    ...
    def detect_query_intent(self, query: str) -> str:
        ...
        summary_words = ["summary", "summarize", "about", "overview"]
        if any(word in query_lower for word in summary_words):
            return "summary"
        key_words = [
            "key points", "main points", "highlights", "important"
        ]
        if any(word in query_lower for word in key_words):
            if "financial":
                ...
                return "conditions"
        conclusion_words = ["conclusion", "result", "outcome"]
        if any(word in query_lower for word in conclusion_words):
            return "conclusion"
        metric_words = ["metrics", "numbers", "data", "statistics"]
        if any(word in query_lower for word in metric_words):
            return "metrics" if "financial" else "findings"
        med_words = ["medication", "drugs", "treatment", "prescription"]
        if any(word in query_lower for word in med_words):
            return "medications"
        status_words = ["status", "condition", "health", "current"]
        if any(word in query_lower for word in status_words):
            return "status"
        method_words = ["method", "how", "approach", "process"]
    ...
    def generate_answer(self, query: str, doc_name: str, doc_type: str) -> str:
        ...
        elif (doc_type in MOCK_RESPONSES and
              "summary" in MOCK_RESPONSES[doc_type]):
            return MOCK_RESPONSES[doc_type]["summary"]

        # Fallback generic response
        return (f"Based on the document '{doc_name}', {query.lower()}. "
                f"This is a privacy-preserving response generated securely "
                f"within the TEE environment.")
    ...
    def simulate_tee_processing(self, query_data: Dict) -> Dict:
        ...
        time.sleep(0.5)
        print("  ✓ TEE enclave initialized")
        time.sleep(0.3)
        print("  ✓ Document decrypted inside TEE")
        time.sleep(0.3)
        print("  ✓ AI model loaded in secure memory")
        time.sleep(0.5)
        print("  ✓ Query processed with privacy guarantees")
    ...
    def _generate_mock_attestation(
            self, query_id: str, document_id: str) -> Dict:
    ...
    def run_demo_mode(self):
        ...
        for idx, query_data in enumerate(sample_queries):
            print(f"\nOK Processing Query {idx + 1}/{query_count}")
        ...
        print("Average Processing Time: ~2 seconds")
